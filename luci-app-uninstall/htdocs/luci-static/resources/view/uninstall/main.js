...
return view.extend({
	...
	render: function() {
			...
			(function(){
				...
				if (darkThemeLoadedPromise) {
					darkThemeLoadedPromise.then(function(enabled) {
						// 确保DOM状态与服务器状态一致
						applyDarkThemeState(enabled);
						// 更新按钮UI（此时 window.darkThemeFromArgon 已经设置）
						updateDarkToggleUI(enabled);
					}).catch(function(err) {
						console.error('加载深色模式状态失败:', err);
					});
				}
		...
		var NAME_MAP = {
			...
			'luci-app-filebrowser': _('filebrowser'),
		...
		img.addEventListener('error', function(){ img.src = DEFAULT_ICON; });
			...
			var purgeLabel = E('label', { 'style': 'display:grid; grid-template-columns:18px minmax(100px, max-content) 36px; align-items:center; column-gap:6px; line-height:20px; white-space:nowrap; opacity:' + (isLocked ? '0.5' : '1') + '; cursor:' + (isLocked ? 'not-allowed' : 'default') + ';' }, [ optionIcon(ICON_PURGE), _('删除配置文件'), purgeSwitch ]);
			var depsEl = E('input', { type: 'checkbox', checked: true, 'style': 'display:none;' });
			var depsSwitch = makeSwitch(depsEl, isLocked);
			var depsLabel = E('label', { 'style': 'display:grid; grid-template-columns:18px minmax(100px, max-content) 36px; align-items:center; column-gap:6px; line-height:20px; white-space:nowrap; opacity:' + (isLocked ? '0.5' : '1') + '; cursor:' + (isLocked ? 'not-allowed' : 'default') + ';' }, [ optionIcon(ICON_DEP), _('卸载相关依赖'), depsSwitch ]);
		...
		function uninstall(name, purge, removeDeps, version, clearCache, cleanupDocker) {
			...
			var removeUrl = L.url('admin/vum/uninstall/remove') + '?' +
				...
				return self._httpJson(removeUrl, {
					...
					println('! Error: ' + String(err));
					// 对于Bad Gateway等响应异常，通常操作已经完成了
					// 先等待一段时间，然后直接显示成功
					println(_('⏱️ 服务器响应异常，正在确认状态...'));
					
					setTimeout(function(){
						setProgress(100);
						clearInterval(timer);
						println(_('⚠️ 虽然服务器响应异常，但通常操作已经完成，请刷新页面确认'));
						statusIconEl.textContent = '✓';
						statusIconEl.setAttribute('style', 'display:inline-flex;width:22px;height:22px;background:#dcfce7;color:#065f46;border-radius:999px;align-items:center;justify-content:center;font-weight:700;');
						statusTextEl.textContent = _('操作完成（请刷新确认）');
						statusTextEl.setAttribute('style', 'font-weight:600;color:#065f46;');
						var statusDone = E('div', { 'style': 'display:flex; align-items:center; gap:8px; margin:8px 0 0 0;' }, [
							E('span', { 'style': 'display:inline-flex;width:22px;height:22px;background:#dcfce7;color:#065f46;border-radius:999px;align-items:center;justify-content:center;font-weight:700;' }, '✓'),
							E('span', { 'style': 'font-weight:600;color:#065f46;' }, _('操作完成（请刷新确认）'))
						]);
						log.parentNode.insertBefore(statusDone, log.nextSibling);
						opSuccess = true;
						enableClose();
